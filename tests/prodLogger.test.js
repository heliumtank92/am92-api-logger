...
const splatObject = { a: { b: { c: { panNumber: 'abcsdhaskdhfkashfkashkfhk' } } } }
class Test {
  a = 123
  test () {
    console.log('a')
...
describe('Test Prod Logs', () => {
  ...
  testIf('false')('Test Splat in production mode', async () => {
    ...
    logger.log(splatError, 'Test Splat Error')
  ...
  testIf('false')('Test Object in production mode', async () => {
    process.env.NODE_ENV = 'production'
    const { default: logger } = await import('../src/logger.mjs')
    const logSpy = jest.spyOn(logger, 'log')
    if (!enableLogging) {
    ...
    // expect(logSpy).toHaveBeenCalledWith(splatObject)
    ...
    // expect(logSpy.mock.calls).toContainEqual([splatObject])
